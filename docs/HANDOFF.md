...
- 계정 관리 동작 (탭, 최대 2개, 독립 저장)
- 로컬 인증 (이메일+비번) 동작
- 데이터 파싱 (일일 10회 제한, 2시간 간격, 05:00 KST 리셋)
- 자동 파싱 (30분마다 체크, 2시간 경과 시 실행, 서버 켜져 있을 때만)
- AI 연결 버튼 (홈 하단, Groq + OpenRouter 키 입력)
- 플레이 창 (오버레이)
  · 창 이동: 상단 56px 드래그 바
  · 투명도: 4단계 순환 (0.95/0.75/0.55/0.35)
  · 항상 위: 토글
...
3. `saveAccount`가 uid 날리는 버그 가능성 (계정 2 복구 경험)
